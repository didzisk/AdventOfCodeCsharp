...
namespace Day3
{
	class MainCalc
	{
		...
		public static void Calc()
		{
			...
			current.X = 0;
			current.Y = 0;
			current.WireLength = 0;
			foreach (var item2 in input2)
			{
				var dir2 = item2[0];
				var count2 = int.Parse(item2.Substring(1));
				switch (dir2)
				{
					case 'U':
			...
			var p1 = V1.Intersect(V2, new PComparer()).OrderBy(p => Math.Abs(p.X) + Math.Abs(p.Y)).First();

			var p2 = V1.Intersect(V2, new PComparer()).OrderBy(p => p.WireLength).First();

			Console.WriteLine($"Part1 ({p1.X}, {p1.Y}) Manhattan Distance {Math.Abs(p1.X) + Math.Abs(p1.Y)}");

			Console.WriteLine($"Point2 ({p2.X}, {p2.Y}) wire length {p2.WireLength}");

			var p3 = V1.First(p => p.X == p2.X && p.Y == p2.Y);
			var p4 = V2.First(p => p.X == p2.X && p.Y == p2.Y);
			Console.WriteLine($"Result ({p3.X}, {p3.Y}) dist {p3.WireLength+p4.WireLength}");
